...
for n = 1:50
    % Use Ackley as the objective function
    % Random initialization of 100 particles in the range [-32, 32] dimension 10
    partcles_SPSO = rand(100, 10) * 64 - 32;
    partcles_CPSOS = rand(100, 10) * 64 - 32;
    ...
    temp1_SPSO = zeros(100, 1);
    tmep2_SPSO = zeros(100, 1);
    temp1_CPSOS = zeros(100, 1);
    tmep2_CPSOS = zeros(100, 1);

    for i = 1:10
        temp1_SPSO = temp1_SPSO + partcles_SPSO(:, i) .^ 2;
        tmep2_SPSO = tmep2_SPSO + cos(2 * pi * partcles_SPSO(:, i));
        temp1_CPSOS = temp1_CPSOS + partcles_CPSOS(:, i) .^ 2;
        tmep2_CPSOS = tmep2_CPSOS + cos(2 * pi * partcles_CPSOS(:, i));
    end

    fitness_SPSO = -20 * exp(-0.2 * sqrt(temp1_SPSO ./ 10)) - exp(tmep2_SPSO ./ 10) + 20 + exp(1);
    fitness_CPSOS = -20 * exp(-0.2 * sqrt(temp1_CPSOS ./ 10)) - exp(tmep2_CPSOS ./ 10) + 20 + exp(1);
    ...
    for i = 1:999
        ...
        for j = 1:10
            temp1_SPSO = temp1_SPSO + partcles_SPSO(:, j) .^ 2;
            tmep2_SPSO = tmep2_SPSO + cos(2 * pi * partcles_SPSO(:, j));
            temp1_CPSOS = temp1_CPSOS + partcles_CPSOS(:, j) .^ 2;
            tmep2_CPSOS = tmep2_CPSOS + cos(2 * pi * partcles_CPSOS(:, j));
        end
...
legend('SPSO', 'CPSOS');
title('Average best fitness of all iterations of SPSO and CPSOS (Ackley)');
