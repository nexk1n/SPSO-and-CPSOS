...
for n = 1:50
    ...
    for i = 1:10
        fitness_SPSO = fitness_SPSO + i * partcles_SPSO(:, i) .^ 2;
        fitness_CPSOS = fitness_CPSOS + i * partcles_CPSOS(:, i) .^ 2;
    end
    ...
    for i = 1:999
        ...
        for j = 1:10
            fitness_SPSO = fitness_SPSO + j * partcles_SPSO(:, j) .^ 2;
            fitness_CPSOS = fitness_CPSOS + j * partcles_CPSOS(:, j) .^ 2;
        end
...
legend('SPSO', 'CPSOS');
title('Average best fitness of all iterations of SPSO and CPSOS (Sum Square)');
